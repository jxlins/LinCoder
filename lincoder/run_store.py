...
class RunStore:
    ...
    def _write_json_atomic(self, path, payload):
        ...
        痛点：在写入 JSON 文件时，如果程序在写入过程中崩溃（如断电、OOM），文件会被截断，留下一个残缺的 JSON。下次读取时会导致 JSONDecodeError，进而引发连锁崩溃。

        解决方案：代码采用了经典的“写临时文件 + 重命名”策略。
            先将数据写入同目录下的 .tmp 临时文件。
            写入完成后，使用 Path.replace() 将临时文件原子性地替换为目标文件。

        工程价值：在绝大多数现代操作系统中，同目录下的文件重命名是一个原子操作。这保证了 task_state.json 和 report.json 永远只有两种状态：
                要么是完整的旧版本，要么是完整的新版本，绝不会出现“写了一半”的脏数据。
