...
'''
模拟真实的模型客户端
让开发者可以在不消耗真实算力、不产生 API 费用、不依赖外部网络的情况下，对调用模型的业务逻辑代码进行单元测试。
'''
class FakeModelClient:
    def __init__(self, outputs):
        self.outputs = outputs      # 预设输出队列
        self.prompts = []           # 记录输入
        self.supports_prompt_cache= False
        self.last_completion_metadata = {}  # 元数据模拟
...
'''
一个真实的生产级客户端
通过底层的 HTTP 请求直接与本地运行的 Ollama 服务进行通信。
'''
class OllamaModelClient:
    ...
    def complete(self, prompt, max_new_tokens, **kwargs):
        ...
        # 直接使用 Python 标准库中的 urllib.request 向 Ollama 的 /api/generate 端点发送 POST 请求
...
def _extract_openai_text(data):
    ...
    功能：从非流式（Non-streaming）的 JSON 响应体中提取纯文本。
    兼容性设计：
        新版 Responses API：优先检查 output_text 字段，或遍历 output -> content 列表提取文本。
        传统 Chat Completions API：回退到检查 choices.message.content，并兼容了 content 可能是字符串或列表（多模态/结构化输出）的情况3。
    防御性编程：通过层层 .get() 和类型检查（isinstance），避免了因为某个字段缺失或类型不符而导致程序抛出 KeyError 或 TypeError。
    '''
...
def _extract_openai_text_from_sse(body_text):
    ...
    功能：解析 Server-Sent Events (SSE) 格式的文本流，提取大模型“打字机”效果生成的最终文本。
    解析逻辑：
        逐行扫描，仅处理以 data: 开头的行，跳过 [DONE] 结束信号。
        将 JSON 载荷解析后，根据 type 字段进行路由：
            如果是 response.output_text.delta，将增量文本（delta）追加到列表中。
            如果是 response.output_text.done 或包含完整的 part/item/response，直接提取完整文本并返回。
        兜底机制：如果流中没有明确的完成信号，函数会将收集到的所有 deltas 拼接起来返回；或者尝试从最后接收到的 response 对象中提取文本。
    '''
    last_response = None
    deltas = []
...
'''
一个非常成熟、生产级别的通用客户端实现
封装了底层的 HTTP 通信细节，还巧妙地处理了当前大模型生态中API 接口不统一、响应格式碎片化（JSON vs SSE）以及高级特性（如 Prompt Caching）的兼容性问题。
'''
